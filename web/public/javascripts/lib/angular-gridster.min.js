...
.controller('GridsterCtrl', ['gridsterConfig',
  function(gridsterConfig) {
    ...
    console.log("Gridster controller running");
    /**
      ...
      for (var rowIndex = 0; rowIndex < this.options.maxRows; ++rowIndex) {
        for (var colIndex = 0; colIndex < this.options.columns; ++colIndex) {
          ...
          if (!this.getItem(rowIndex, colIndex) && this.canItemOccupy(item, rowIndex, colIndex)) {
            console.log("autoSetItemPosition:: putting item in", rowIndex, colIndex);
    ...
    this.putItems = function(items) {
      console.log("putItems", "total of " + items.length + " items");
     ...
     */
    this.putItem = function(item, row, column, resolveOverlap) {
      console.log("putItem", "item: ", item.row, item.col, " params:", row, column);

      if (typeof resolveOverlap === 'undefined' || resolveOverlap === null) {
        resolveOverlap = true;
      }

      // If row was not passed as parameter, take it from item
      if (typeof row === 'undefined' || row === null) {
        row = item.row;
        column = item.col;
        // If item has no row, autoposition it
        if (typeof row === 'undefined' || row === null) {
      ...
      // If item was being moved
      if (item && item.oldRow !== null && typeof item.oldRow !== 'undefined') {
        // Item actually remained in place
        if (item.oldRow === row && item.oldColumn === column) {
          ...
          if (oldRow && oldRow[item.oldColumn] === item) {
            console.log("!!! removed item from row", item.oldRow, " column ", item.oldColumn);
      ...
      // Add empty row to the grid, if needed
      if (!this.grid[row]) {
        this.grid[row] = [];
      }

      // put the item into position
      console.log("Put item in GRID row", item.oldRow, " column ", item.oldColumn);
    ...
    this.moveOverlappingItems = function(item) {
      console.log("Move overlapping items");
      if (item.oldRow)
        console.log("Item was moved", "oldRow=", item.oldRow, "oldColumn=", item.oldColumn, "newRow=", item.row, "newCol=", item.col);
      console.log('Grid before overlap resoliution', this.grid);
      ...
      console.log("Overlapping items", items);

      // If conflict exists, resolve it
      if (items && items.length > 0) {
        console.log("Conflict detected with " + items.length + " items");

        var moveRight = (item.oldRow > item.row || item.oldColumn > item.col);
        ...
        {
          var t = items[j];
          var newCol = moveRight ? t.col + item.sizeX : t.col - item.sizeX;
          console.log("Move item ", item, " to row", t.row, " column: ", newCol);
          this.putItem(t, t.row, moveRight ? t.col + 1 : t.col - 1, false);
        }
      }

      // Update old row and column at the end of conflict resolution
      item.oldRow = item.row;
      item.oldColumn = item.col;

      console.log('Grid after overlap resolution', this.grid);
    };
...
.controller('GridsterItemCtrl', function() {
  ...
  this.setPosition = function(row, column) {
    console.log("setPosition:: row = ", row, " column = ", column)
  ...
  this.setSize = function(key, value) {
    ...
    if (changed) {
      console.log("Calling moveOverlappingItems from setSize");
