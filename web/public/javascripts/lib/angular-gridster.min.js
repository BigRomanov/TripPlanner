...
.controller('GridsterCtrl', ['gridsterConfig',
  function(gridsterConfig) {
    ...
    this.putItem = function(item, row, column) {
      ...
      console.log("putItem", "from: ", item.row, item.col, " to:", row, column)
      ...
      console.log("!!! put item in new position. row", item.oldRow, " column ", item.oldColumn);
    ...
    this.moveOverlappingItems = function(item) {
      ...
      if (items && items.length > 0) {
        ...
        {
          var t = items[j];
          this.putItem(t, t.row, moveRight ? t.col + 1 : t.col - 1);
        }
      }
    ...
    /**
     * Prevents items from being overlapped - by moving all items in the column down
     *
     * @param {object} item The item that should remain
     */
    this.moveOverlappingItems_old = function(item) {
      var items = this.getItems(
        item.row,
        item.col,
        item.sizeX,
    ...
    this.moveItemsRight = function(items, row) {
      if (!items || items.length === 0) {
        return;
      }
    };
    ...
    this.moveItemsDown = function(items, row) {
      console.log("moveItemsDown", items, row);
      if (!items || items.length === 0) {
        return;
      }
      var topRows = {},
    ...
    return {
      ...
      compile: function($elem) {
        ...
        return function(scope, $elem, attrs, controller) {
          ...
          $timeout(function() {
            console.log("Floating all items up ====================")
...
.controller('GridsterItemCtrl', function() {
   ...
   * @param {number} column
   */
  this.setPosition = function(row, column, oldRow, oldColumn) {
    ...
    return {
      ...
      link: function(scope, $el, attrs, gridster) {
        ...
        function positionChanged() {
          item.setPosition(item.row, item.col, item.oldRow, item.oldColumn);
